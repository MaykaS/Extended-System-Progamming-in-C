...
void list_print(node *diff_list, FILE* output, int f_n_diff){
    int i=1;
    
    node *current= diff_list; /*pointer to the beginning*/
    while(current && i<=f_n_diff){ /*as long as there is a current*/
...
node* restore(FILE * new_file, node *diff_list, int r_n_diff){
    int i=1;
    node *current=diff_list;
    while(current && i<=r_n_diff){
        fseek(new_file, current->diff_data->offset, SEEK_SET);
        unsigned char ToChange = current->diff_data->orig_value;
        fwrite( &ToChange, sizeof(ToChange), 1, new_file);
        current=current->next;
    }
    return current;
}
...
int main(int argc, char **argv){
    ...
    int t_flag=0;
    int diff_counter=0;
    int f_n_diff=INT_MAX; /*print first n diff*/
    int r_flag=0;
    int r_n_diff=INT_MAX; /*restore first n diff*/
    
    
    int idx;
    for (idx = 3; idx < argc;  idx++) {
        if (strcmp(argv[idx], "-o") == 0) {
            output=fopen(argv[idx+1],"w");
            idx++;
        }else if (strcmp(argv[idx], "-t") == 0) {
            t_flag=1;
        }else if (strcmp(argv[idx], "-k") == 0) {
            f_n_diff=atoi(argv[idx+1]);
            idx++;
        }else if (strcmp(argv[idx], "-r") == 0) {
            r_flag=1;
            if(idx+1<argc){
                if(atoi(argv[idx+1])!=0)
                    f_n_diff=atoi(argv[idx+1]);
            }
            idx++;
        }
    ...
    if(r_flag==1)
        head= restore(new_file,head,r_n_diff);
    
    list_print(head,output,f_n_diff);
    
    if(t_flag==1)
        fprintf(output, "total diff: %d\n",diff_counter);
    fclose(output);
    list_free(head);
    return 0;
}
